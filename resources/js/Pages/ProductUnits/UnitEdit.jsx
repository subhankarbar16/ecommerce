...
export default function UnitEdit({ auth,unit }) {
    ...
    const { data, setData, put, processing, errors, reset } = useForm(unit);
    const submit = (e) => {
        e.preventDefault();
        console.log(data);
        put(route('productunits.update',unit.id));
    };

    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Product Unit (Edit)</h2>}
        >
            <Head title="Product Unit" />
