...
export default function BannerEdit({ auth,banner,sorting_orders }) {
console.log(banner);
    const { data, setData, post, processing, errors, reset } = useForm(banner);
    const submit = (e) => {
        e.preventDefault();
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Banners (Edit : <img className="inline w-10" src={`/images/banners/${banner.banner_image}`} /> {banner.title} )</h2>}
            >
            <Head title="Banners" />
